$(function() {
    ...
    let isADoubleClick: boolean = false;
    ...
    let numOfTasks: any = new Array($('.task-list__item'));
    let standardMsg: string = `<p class="task-dashboard__msg">Welcome, you have ${numOfTasks[0].length} task(s) pending!</p>`;
    ...
    ::::*/
    $(document).on('click', 'li', function(e: object) {
        //Storing <ul> in a variable for reference
        let list: string = $(this).parent().html();
        /*::::
        ...
        if(list.indexOf('input') != -1) {
            let editActiveMsg: string = `<p class="task-dashboard__msg">You are still editing a task!</p>`;
            ...
            ::::*/
            let clickedLiValue: string = $(this).text().trim();
            let editForm: string = `<input type="text" id="edit-field" name="edit-field" size="20" value="${clickedLiValue}" />`;
        ...
        if(isADoubleClick) {
            let removalMsg: string = `<p class="task-dashboard__msg">Removed an old task!</p>`;
    ...
    $('#add-task-form').on('submit', function(e) {
        ...
        e.preventDefault();
        let formValue: any = $('#addition-field').val();
        let addedMsg: string = `<p class="task-dashboard__msg">Added a new task!</p>`
        if (dashboard.children().is('.task-dashboard__msg')) {
